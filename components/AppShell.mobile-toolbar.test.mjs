...
test("keeps the mobile action layer open after using an expanded action", () => {
  ...
  assert.match(source, /handleSystemInfoToggle\("tools", mobile\)[\s\S]*?data-mobile-toolbar-action=\{mobile \? "tools" : undefined\}[\s\S]*?onClick=\{handleOpenSessionMap\}[\s\S]*?data-mobile-toolbar-action=\{mobile \? "session-map" : undefined\}/);
