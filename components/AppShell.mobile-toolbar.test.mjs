...
test("uses a compact narrow-mobile toolbar with a floating action layer", () => {
  ...
  for (const action of ["history", "name", "agents", "branches", "system", "tools", "session-map", "theme", "language"]) {
...
test("keeps the mobile action layer open after using an expanded action", () => {
  ...
  assert.match(source, /handleSystemInfoToggle\("system", mobile\)[\s\S]*?data-mobile-toolbar-action=\{mobile \? "system" : undefined\}[\s\S]*?handleSystemInfoToggle\("tools", mobile\)[\s\S]*?data-mobile-toolbar-action=\{mobile \? "tools" : undefined\}[\s\S]*?onClick=\{handleOpenSessionMap\}[\s\S]*?data-mobile-toolbar-action=\{mobile \? "session-map" : undefined\}/);
