...
test("normal session teardown paths use graceful extension shutdown", async () => {
  ...
  assert.match(forkSource, /const replacement = command\.type === "fork_after" \? "fork_after" : "fork"/);
  assert.match(forkSource, /shutdownAfterSessionReplacement\(replacement\)/);
...
test("fork variants and tree navigation reject every running session state", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const forkSource = source.slice(source.indexOf('case "fork"'), source.indexOf('case "navigate_tree"'));
  const navigateSource = source.slice(source.indexOf('case "navigate_tree"'), source.indexOf('case "set_thinking_level"'));

  assert.match(source, /SESSION_REPLACEMENT_COMMAND_TYPES = new Set\(\["fork", "fork_after", "clone"\]\)/);
  assert.match(source, /isSessionRunningForReplacement\(\)[\s\S]*?promptAdmissionPendingCount > 0/);
  assert.match(forkSource, /case "fork_after"/);
  assert.match(forkSource, /command\.type === "fork_after" \? entry\.id : entry\.parentId/);
  assert.match(forkSource, /if \(this\.isSessionRunningForReplacement\(\)\)/);
  assert.match(navigateSource, /if \(this\.isRunning\(\)\)/);
});
