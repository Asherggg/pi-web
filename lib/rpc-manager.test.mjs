...
test("fork and tree navigation reject every running session state", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  ...
  assert.match(source, /if \(type === "fork" \|\| type === "fork_after" \|\| type === "navigate_tree"\)[\s\S]*?if \(this\.isRunning\(\)\)/);
  assert.match(forkSource, /case "fork_after"/);
  assert.match(forkSource, /command\.type === "fork_after" \? entry\.id : entry\.parentId/);
  assert.match(forkSource, /if \(this\.isRunning\(\)\)/);
